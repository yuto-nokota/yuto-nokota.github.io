...
function createProblem () {
  stopSound();
  let F1=Math.floor(9900*Math.random()+100);
  let F2=Math.floor(9900*Math.random()+100);
  let F3=Math.floor(9900*Math.random()+100);
  document.getElementById('B1').value=F1+"[Hz]";
  document.getElementById('B2').value=F2+"[Hz]";
  document.getElementById('B3').value=F3+"[Hz]";
  var rnd = Math.floor ( 3 * Math.random() );
  if ( rnd === 0 ) { B1=correct; B2=wrong;   B3=wrong;   startSound(F1); }
  if ( rnd === 1 ) { B1=wrong;   B2=correct; B3=wrong;   startSound(F2); }
  if ( rnd === 2 ) { B1=wrong;   B2=wrong;   B3=correct; startSound(F3); }
}
